...
<div id='quarterly' class='orders <%=(freq!=null&&freq.equals(Frequency.Q.name()))?"target":"" %>'>
	...
	<%if(cart==null || indexSet==null || indexSet.isEmpty()){ %>
		<p class="error">查無<%=Frequency.Q.getDescription() %>(No quarterly items found)</p>
	<%}else{ %>
		...
		<section class="selection">
			...
			<div class="delete">
				...
				<input class='delete' type='submit' name='deleteAll' data-freq='<%=Frequency.Q.getDescription() %>' value='刪除所有<%=Frequency.Q.getDescription() %> ( Delete ALL )'>
			</div>
		</section>
		</form>
